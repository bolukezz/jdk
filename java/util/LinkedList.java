    ...
    implements List<E>, Deque<E>, Cloneable, java.io.Serializable
{
    ...
    private E unlinkFirst(Node<E> f) {
       ...
       //让node的元素置为空
        ...
        //让first指针指向next节点
        first = next;
        //如果next节点为空，说明当前链表内没有节点，让last也设置为空
        if (next == null)
            last = null;
        else
            //如果next节点不为空，让next节点的prev指向空，因为已经把之前的头节点从链表内删除了
            next.prev = null;
        //个数--
        size--;
        //修改次数++
    ...
    private E unlinkLast(Node<E> l) {
        ...
        //让尾节点的prev设置为空
        l.prev = null; // help GC
        //让last节点设置为之前尾节点的prev
        last = prev;
        //如果prev为空，说明之前只有一个节点，那么把first节点也设置为空
        if (prev == null)
            first = null;
        else
            //如果prev不为空，那么让prev的next设置为空。
            prev.next = null;
        //个数--
        size--;
        //修改次数++
