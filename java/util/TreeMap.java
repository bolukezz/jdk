...
public class TreeMap<K, V>
    ...
    /**
     * 因为treeMap的存储结构是红黑树，所以必然有一个唯一的跟节点
     */
     ...
     * cpr.compare(key, t.key)比较传入的key和根节点的key值，如果传入的key
    ...
    public V put(K key, V value) {
        Entry<K, V> t = root;
        /**
         * 在这里判断是否是根节点为null，如果根节点也为null说明红黑树还没建立起来，那么首先new一个Entry，并赋值给root把树给建立起来
         * 这个时候红黑树已经有一个节点了，同时修改操作++
         */
        if (t == null) {
        ...
        /**
         * 如果root节点不为null，定义一个cmp，这个变量用来进行二分查找时候的比较，是new Entry时候必须要的参数
         */
        int cmp;
        Entry<K, V> parent;
        // split comparator and comparable paths
        // cpr表示是否有无自己的排序规则，分两种情况遍历执行
        Comparator<? super K> cpr = comparator;
        if (cpr != null) {
            /**
             * 从root节点开始遍历，通过二分查找逐步向下找，
             * 第一次循环：从跟节点开始，这个时候parent就是根节点，然后通过自定义的排序算法
             * cpr.compare()比较传入的key和根节点的key，如果传入的key>root.key,那么继续在root
             * 的右节点中寻找，如果传入的key<root.key，那么在root的左节点中寻找，如果key==root.key,
             * 那么直接替换root节点的value即可。
             */
            do {
                parent = t;
            ...
            // 从这里可以看出，如果是默认排序的情况下，key值是不能为null的
            if (key == null)
                throw new NullPointerException();
            @SuppressWarnings("unchecked")
            Comparable<? super K> k = (Comparable<? super K>) key;
            // 这边的实现逻辑也是同上，都是二分查找的逻辑。
            do {
                parent = t;
        ...
        /**
         * 执行到这里，说明没有找到相同的key，节点已经遍历到最后了，我们需要新new一个ENtry放到parent下面即可,
         * 这里是放到左节点还是右节点就需要按照红黑树的逻辑来了。
         */
        ...
        if (cmp < 0)
            ...
            parent.right = e;
        /**
         * 节点加进去了，并不算完，因为新加一个节点，会对红黑树的平衡造成影响，我们需要通过一些操作来进行自动处理平衡，
         */
        fixAfterInsertion(e);
        size++;
        modCount++;
    ...
    abstract static class NavigableSubMap<K, V> extends AbstractMap<K, V>
        ...
        final TreeMap.Entry<K, V> absLowest() {
            TreeMap.Entry<K, V> e =
                    (fromStart ? m.getFirstEntry() : (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
            return (e == null || tooHigh(e.key)) ? null : e;
        }

        final TreeMap.Entry<K, V> absHighest() {
            TreeMap.Entry<K, V> e =
                    (toEnd ? m.getLastEntry() : (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
    ...
    private void fixAfterInsertion(Entry<K, V> x) {
        ...
        // 我们知道父节点为黑色节点时，不需要进行树结构调整，只有当父节点为红色时，才需要调整
        while (x != null && x != root && x.parent.color == RED) {
            // 如果父节点是左节点，对应上面的情况1和情况2
            if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {
                Entry<K, V> y = rightOf(parentOf(parentOf(x)));
                // 如果叔父节点为红色，对应"父节点和叔父节点都为红色"，即可通过变色实现
                ...
                if (colorOf(y) == RED) {
                    ...
                    // 如果插入的节点是黑色，插入的是左子节点，【通过左右旋转】 (这里先进行父节点右旋)
     ...
     * 待删除节点的左 右子节点都为null，删除时将该节点置为null;
     * 待删除节点的左右子节点有一个有值，则用有值的节点替换该节点即可；
     * 待删除节点的左右子节点都不为null，则找前驱或者后继，将前驱或者后继的值复制到该节点中，
