...
public class WeakHashMap<K, V>
    ...
    private void expungeStaleEntries() {
        // 遍历对应的ReferenceQueue,因为现在在这个队列里entry对应的key已经被gc回收了。
        for (Object x; (x = queue.poll()) != null;) {
            // 在这里进行加锁，为了防止通过多个线程同时清除
            synchronized (queue) {
                @SuppressWarnings("unchecked")
                Entry<K, V> e = (Entry<K, V>) x;
                // 从队列中获取出来对应的Entry，然后计算出来对应的slot
                int i = indexFor(e.hash, table.length);

                Entry<K, V> prev = table[i];
                Entry<K, V> p = prev;
                // 遍历对应的slot，然后从table中清除在队列中已经过期的entry
                while (p != null) {
    ...
    public V get(Object key) {
        ...
        int h = hash(k);
        //发现都是get还是put都是类似的，但是唯一不同的就是在这里getTable回去弱引用队列中获取需要过期的Entry，然后过期即可。
    ...
    public V put(K key, V value) {
        // 这里是判断如果key为null，则使用NULL_KEY来代替
        Object k = maskNull(key);
        // 获取对应的hashcode，在这里是通过位移和或来进行降低hash冲突的
        int h = hash(k);
        // 获取对应的table，在获取的时候还会从遍历引用队列，然后释放掉过期的Entry
        Entry<K, V>[] tab = getTable();
        // 获取对应的slot
        int i = indexFor(h, tab.length);
        //从slot中的头节点然后遍历整个链表，判断是不是已经存在的key，如果存在直接替换对应的value即可
        ...
        modCount++;
        Entry<K, V> e = tab[i];
        //走到这里说明没有存在的key，说明需要新建一个Entry
        tab[i] = new Entry<>(k, value, queue, h, e);
        //判断是否到达了阈值，如果到达了则调用对应的resize
        if (++size >= threshold)
            resize(tab.length * 2);
        return null;
    }
