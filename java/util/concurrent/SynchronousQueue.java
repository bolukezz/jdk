...
public class SynchronousQueue<E> extends AbstractQueue<E>
     ...
     */
    //针对有超时的情况，自旋了多少次后，如果剩余时间大于1000纳秒就是用带时间的LockSupport.parkNanos()这个方法
    ...
    static final class TransferStack<E> extends Transferer<E> {
            ...
            for (;;) {
                ...
                if (h == null || h.mode == mode) {  // empty or same-mode
                    ...
                    if (h.isCancelled())            // already cancelled
                        ...
                        //头节点没有在匹配中，就让当前节点先入队，再让他们尝试匹配且s成为了新的头节点，它的状态是正在匹配中
                        for (;;) { // loop until matched or waiters disappear
                            SNode m = s.next;       // m is s's match
                            //如果m为null，说明除了s节点外的节点都被其他线程先一步匹配掉了
                            ...
                            //尝试匹配失败，说明m已经先一步被其他线程匹配了
                            //就协助清除他
                                s.casNext(m, mn);   // help unlink
                        }
                    }
                } else {
                    //到这里说明当前节点模式和头节点模式不一样
                    //且头节点是正在匹配中
                    // help a fulfiller
                    SNode m = h.next;               // m is h's match
                    if (m == null)                  // waiter is gone
                        //如果m为null，说明m已经被其他线程先一步匹配了。
                        ...
                        //尝试匹配失败，说明m已经先一步被其他线程匹配了，就协助清除它
        ...
        SNode awaitFulfill(SNode s, boolean timed, long nanos) {
            ...
            for (;;) {
                ...
                if (w.isInterrupted())
                    s.tryCancel();
                //检查s是否匹配到了元素m(有可能是其他线程的m匹配到了当前线程的s)
                SNode m = s.match;
                //如果匹配到了，直接返回m
                if (m != null)
                    return m;
                ...
                //后面的elseif都是自旋次数没有了
                    ...
                    //如果不允许超时，直接阻塞，并等待被其他线程唤醒，唤醒后继续自旋并查看是否匹配到了元素
    ...
    public void put(E e) throws InterruptedException {
        ...
        //直接调用传输器的transfer()方法，三个参数分别是：传输的元素，是否需要超时，超时的时间
