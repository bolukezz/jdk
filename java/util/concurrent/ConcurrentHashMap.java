...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0)
                // 如果sizeCtl<0说明正在初始化或者正在扩容，让出CPU
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                // 如果把sizeCtl原子更新为-1成功，则当前线程进行初始化
                // 如果原子更新失败则说明有其他线程先一步进入初始化了，则进入下一次循环
                // 如果下一次循环时还没有初始化完毕，则sizeCtl<0进入上面if的逻辑让出CPU
                ...
                try {
                    ...
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        // 赋值给table桶数组
                        table = tab = nt;
                        // 设置sc为数组长度的0.75倍 n-(n>>>2)=n-n/4=0.75
                        // 这里装载因子和扩容预置都是写死的，这也是没有threshold和loadFactor属性的原因
                        sc = n - (n >>> 2);
                    }
                } finally {
    ...
    final Node<K, V>[] helpTransfer(Node<K, V>[] tab, Node<K, V> f) {
        ...
        // 说明当前桶已经迁移完毕了，才去帮忙迁移其他桶的元素
        // 扩容时会把旧桶的第一个元素设置为ForwardingNode,并让其nextTab指向新桶数组
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        for (int i = 0, bound = 0;;) {
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                // 如果桶中无数据，直接放入ForwardingNode标记该桶已经迁移
                advance = casTabAt(tab, i, null, fwd);
            else if ((fh = f.hash) == MOVED)
                // 如果桶中第一个元素的hash为MOVED
    ...
    static final class KeySpliterator<K, V> extends Traverser<K, V>
        ...
        public Spliterator<K> trySplit() {
            int i, f, h;
            return (h = ((i = baseIndex) + (f = baseLimit)) >>> 1) <= i
                    ? null
