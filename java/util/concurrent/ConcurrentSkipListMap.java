...
public class ConcurrentSkipListMap<K, V> extends AbstractMap<K, V>
     ...
     */
    /**
     * 初始化了一些属性，并创建了一个头节点，里面存储了一个数据节点，这个数据节点的的值是空对象，且它的层级为1。
     */
        ...
        void helpDelete(Node<K, V> b, Node<K, V> f) {
            ...
            if (f == next && this == b.next) {
                ...
                if (f == null || f.value != f) // not already marked
                    ...
                    //marker过了，就把b的下个节点指向marker的下个没电
     ...
     */
    // 头索引指针，继承自Index，并扩展一个level字段，用于记录索引的层级
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        outer: for (;;) {
            ...
            // 为了便于描述，这里b叫做当前节点，n叫做下一个节点
            for (Node<K, V> b = findPredecessor(key, cmp), n = b.next;;) {
                // 如果下一个节点不为空
                // 就拿其key与目标节点的key做比较，找到目标节点应该插入的位置
                if (n != null) {
                    // v=value，存储节点value值
                    // c=compare，存储两个节点比较的大小
                    Object v;
                    int c;
                    // n的下一个节点数据，也就是b的下一个节点的下一个节点(孙子节点)
                    Node<K, V> f = n.next;
                    /// 如果n不为b的下一个节点
                    // 说明有其他线程修改了数据，则跳出内层循环
                    ...
                    if (c == 0) {
                        ...
                        // 如果替换旧值失败，说明其他线程先一步修改了值，从头来过
                        break; // restart if lost race to replace value
                    }
                    // 如果c<0,就往下走，也就是找到了目标节点的为孩子
                ...
                // 如果c<0，则n的key比目标key大，相妆于在b和n之间插入目标节点
                z = new Node<K, V>(key, value, n);
                // 原子更新b的下一个节点为目标节点z
                if (!b.casNext(n, z))
                    // 如果更新失败，说明其他线程先一步修改了值，从头来过
        ...
        if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
            //默认level为1，也就是只要到了这里了就会至少建立一层索引
            int level = 1, max;
            ...
            if (level <= (max = h.level)) {
                ...
                //如果新的层数超过了现有跳表的高度
                //则最多值增加一层
                //比如现在只有一层索引，那下一次最多增加了两层索引，增加多了也没有意义。
            ...
            splice: for (int insertionLevel = level;;) {
                ...
                for (Index<K, V> q = h, r = q.right, t = idx;;) {
                    ...
                    //第一个if
                    //j与insertionLevel相等了，
                    //实际上是先走的第二个if，j自减后应该与insertionLevel相等
                    if (j == insertionLevel) {
                        //这里是真正连接右指针的地方
                        if (!q.link(r, t))
                            //连接失败，从头来过
                            break; // restart
                        //t节点的值为空，可能是其他线程删除了这个元素
    ...
    final V doRemove(Object key, Object value) {
        ...
        outer: for (;;) {
            ...
            //为了方便，这里把b叫做当前节点，n为下一个节点，f为下下个节点
            for (Node<K, V> b = findPredecessor(key, cmp), n = b.next;;) {
                Object v;
                int c;
                ...
                //c=0,说明n就是要找的元素
                //如果value不为空且不等于找到的元素的value，不需要删除，退出外层循环
    ...
    public V put(K key, V value) {
        ...
        // 因为 value 为 null 标记该元素被删除(后面会看到)
        if (value == null)
            throw new NullPointerException();
        // 调用 doPut() 方法添加元素
     ...
     *
     * <p>
     * The {@code Map.Entry} elements traversed by the {@code iterator}
