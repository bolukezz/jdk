 ...
 */
//这里LinkedTransferQueue使用了一个叫做双重数据结构或者是双重队列。
    //双重队列的意思是放取元素使用同一个队列，队列的节点具有2种模式，一种是数据节点，一种是非数据节点
    //放元素时先跟头节点做比较，如果头节点是非数据节点，就让他们匹配，如果头节点是数据节点，就生成一个数据节点在队列尾端(入队)
    //取元素时也是先根队列头节点比较，如果头节点是数据节点，就让他们匹配，如果头节点是非数据节点，就生成一个非数据节点放在队列尾端(入队)
...
public class LinkedTransferQueue<E> extends AbstractQueue<E>
    ...
    //同步，调用的时候如果没有匹配到会一直阻塞到匹配为止
    private static final int SYNC = 2; // for transfer, take
    //超时，有用于超时的poll()和tryTransfer()方法中
     ...
     */
    //1。e表示元素 2。haveData表示是否是数据节点 3。how表示放取元素的方式，上面提到的四种NOW、ASYNC、SYNC、TIMED ，nanos表示超时时间
