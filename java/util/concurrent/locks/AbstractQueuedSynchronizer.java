...
public abstract class AbstractQueuedSynchronizer
    ...
    private void unparkSuccessor(Node node) {
         ...
         */
        Node s = node.next;
        // 判断当前节点是如果不满足唤醒的条件，那么就遍历他的后续找到对应满足的节点
    ...
    private void doReleaseShared() {
         ...
         */
        // 在这里是一个for循环，说明他会唤醒后续所有可以满足条件的节点
        for (;;) {
            // 在这里释放锁之后唤醒后继者
            Node h = head;
            // 这里判断自己是不是最后一个节点，如果不是最后一个节点，那说明在队列中还有节点，那么需要在释放锁的时候唤醒后续节点
            if (h != null && h != tail) {
                // 在这里就发现了，只有节点的状态是SIGNAL状态才代表节点在释放锁的时候会唤醒后继节点
                int ws = h.waitStatus;
                if (ws == Node.SIGNAL) {
                    // 使用CAS将当前节点的status设置为0，如果CAS成功，则唤醒后续节点
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // 在阻塞的时候要确保前一个节点的status是SIGNAL，SIGNAL保证在释放锁的时候会唤醒后续节点。
        if (ws == Node.SIGNAL)
            /*
             ...
             */
            return true;
        // 前一个节点的状态大于0，只有一种状态为cancel。
        if (ws > 0) {
            /*
             * Predecessor was cancelled. Skip over predecessors and
             * indicate retry.
             */
            // 在这里循环跳过所有前面状态为cancel状态的node
            do {
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            pred.next = node;
             ...
             */
            // 把前一个节点的状态设置为SIGNAL,代表释放锁的时候会唤醒后续节点.
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                // 获取当前节点的前一个节点，如果前一个节点为head，说明是head节点释放锁唤醒后续节点的操作，那么进行抢占锁，
    ...
    private boolean doAcquireNanos(int arg, long nanosTimeout)
        ...
        // 把当前线程包装成一个node然后在一个for循环中尝试进行加锁
        final Node node = addWaiter(Node.EXCLUSIVE);
        boolean failed = true;
        try {
            for (;;) {
                // 获取当前节点的前一个，然后判断如果是head，则尝试进行加锁
                final Node p = node.predecessor();
                if (p == head && tryAcquire(arg)) {
                    setHead(node);
                    p.next = null; // help GC
    ...
    private void doAcquireShared(int arg) {
        ...
        boolean failed = true;
        // 在一个for循环中park自己，如果被唤醒再重新尝试获取锁，这样一直循环。
        try {
            boolean interrupted = false;
            for (;;) {
                // 获取前一个节点
                final Node p = node.predecessor();
                // 判断前一个节点是head节点,那么他又被唤醒了，说明他是被head唤醒的，那么可以去进行抢锁
                if (p == head) {
                    // 尝试获取读锁
                    int r = tryAcquireShared(arg);
                    // 如果获取到了锁，那么设置头节点为当前节点node并且传播出去，唤醒后续的读节点
                    if (r >= 0) {
                        setHeadAndPropagate(node, r);
                        p.next = null; // help GC
                        // 判断如果在执行过程中有人发出了中断信号，但是因为在执行把线程的中断标记给擦除了，所以在这里再给恢复。
                        if (interrupted)
                            selfInterrupt();
                        failed = false;
                        return;
    ...
    public final void acquire(int arg) {
        // 再次尝试去获取锁，获取不到加入队列，addWaiter会把当前线程包装成一个Node节点。
        if (!tryAcquire(arg) &&
    ...
    public final boolean tryAcquireNanos(int arg, long nanosTimeout)
            throws InterruptedException {
        // 这里发现这个方法是可中断的，如果判断有其他线程发出了中断信号，那么直接抛出中断异常
        if (Thread.interrupted())
            throw new InterruptedException();
        // 尝试获取锁，如果获取不到，那么调用doAcquireNanos尝试在规定时间内获取锁
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        Node h = head;
        Node s;
        // 在这里判断h!=t说明，tail和head已经初始化了，判断h.next为null，说明当前队列没有排队的线程或者队列的第一个就是当前线程。
        return h != t &&
    ...
    final boolean isOnSyncQueue(Node node) {
         ...
         */
        // 从AQS的尾节点开始往前寻找看是否可以找到当前节点，找到了也说明已经在AQS的队列中
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            ...
            // 如果条件队列的尾节点已取消，从头节点开始清理所有已取消的节点
            if (t != null && t.waitStatus != Node.CONDITION) {
                unlinkCancelledWaiters();
                // 重新获取尾节点
                t = lastWaiter;
            }
            // 新建一个节点，他的等待状态是CONDITION
        ...
        public final void await() throws InterruptedException {
             ...
             *
             * 其次，移到AQS的队列中时等待状态会更改为0（AQS队列节点的初始等待状态0）；
             *
