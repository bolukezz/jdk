 ...
 *
 * <p>
 * <b>Sample usages</b>. Here is a code sketch showing how to perform
...
public class ReentrantReadWriteLock
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected final boolean tryRelease(int releases) {
            // 在读写锁释放写锁的时候，判断当前持有独占锁是否是当前线程，如果不是抛错
            if (!isHeldExclusively())
                throw new IllegalMonitorStateException();
            // 在这里释放锁了，对应的state也需要进行--
            int nextc = getState() - releases;
            // 判断是不是可重入锁
            boolean free = exclusiveCount(nextc) == 0;
            // 如果不是可重入所，那么将当前持有锁线程设置为空
            if (free)
        ...
        protected final boolean tryReleaseShared(int unused) {
            ...
            if (firstReader == current) {
                ...
                // 如果不是firstReader的话，则获取当前线程的计数器,然后--
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            Thread current = Thread.currentThread();
            int c = getState();
            // 判断此时如果已经被别人获取了写锁，并且这个获取写锁的人不是自己
            // 这里判断是否有人获取到写锁是通过 c & EXCLUSIVE_MASK; ，当前的state & EXCLUSIVE_MASK = (1 << SHARED_SHIFT) - 1;
            if (exclusiveCount(c) != 0 &&
                    getExclusiveOwnerThread() != current)
                return -1;
            // 获取到当前共享锁的数量
            int r = sharedCount(c);
            // 判断读锁是否应该阻塞，这个是根据是否是公平或非公平锁的策略，判断读锁是否到到最大数量，最后再CAS尝试获取锁
            // 这里为了防止写锁饥荒，在公平锁的情况下都是应该排队的，在非公平锁的情况下为了防止写锁一直拿不到锁，所以判断
            // 当前队列的第一个如果是写锁的话，那么读锁也要去排队。
            if (!readerShouldBlock() &&
                ...
                if (r == 0) {
                    ...
                    HoldCounter rh = cachedHoldCounter;
                    // 获取当前线程的计数，然后判断如果为空则给当前线程set一个计数器，不为空则获取当前线程的计数器
                    if (rh == null || rh.tid != getThreadId(current))
            ...
            // 如果在上面判断该线程可以拿到读锁，并且是cas的时候没有抢到，那没用尝试拿到锁，那么会在下面循环进行获取锁
        ...
        final int fullTryAcquireShared(Thread current) {
            ...
            for (;;) {
                ...
                // 判断如果读锁的数量到达了阈值，则抛出异常
                if (sharedCount(c) == MAX_COUNT)
                    throw new Error("Maximum lock count exceeded");
                // 修改当前state的状态，并且在threadLocal中保存当前线程的计数器
    ...
    public static class WriteLock implements Lock, java.io.Serializable {
         ...
         */
        public void lock() {
            // 获取独占锁,这里独占锁其实适合ReentrantLock走的是相同的逻辑
            sync.acquire(1);
        }
